...
export function setHandlerContext(context: ModuleContext): void {
  ctx = context;
}

export async function findLeadsChannel(): Promise<TextChannel | null> {
...
export function buildLeadEmbed(lead: Lead): EmbedBuilder {
...
export async function handleNearby(interaction: ChatInputCommandInteraction): Promise<void> {
  const lat = interaction.options.getNumber('lat', true);
  const lng = interaction.options.getNumber('lng', true);
  const radiusKm = interaction.options.getInteger('radius') ?? 5;
  ...
  const places = await searchNearby(apiKey, lat, lng, radiusKm, types);
  ...
  for (const place of places) {
    ...
    `).run(
      place.place_id, place.name, place.address, place.phone, place.website,
      place.google_maps_url, place.photo_url, place.rating, place.review_count, place.category,
      `nearby(${lat},${lng},${radiusKm}km)`,
      score.total, score.recommended_service,
    );
    ...
    results.push(ctx.db.prepare('SELECT * FROM leads WHERE id = ?').get(leadId) as Lead);
    await new Promise((r) => setTimeout(r, 200));
  }

  logApiUsage(ctx.db, 'nearbySearch', places.length, 1);

  const embed = new EmbedBuilder()
    .setTitle(`Nearby: ${lat}, ${lng} (${radiusKm}km)`)
    .setColor(0x5865f2)
    .setDescription(
      results.length === 0
        ? 'Todos os resultados já estavam no banco.'
...
export async function handleConfigAdd(interaction: ChatInputCommandInteraction): Promise<void> {
  const query = interaction.options.getString('query', true);
  const region = interaction.options.getString('region', true);
...
export async function handleConfigList(interaction: ChatInputCommandInteraction): Promise<void> {
...
export async function handleConfigRemove(interaction: ChatInputCommandInteraction): Promise<void> {
...
export async function handleCost(interaction: ChatInputCommandInteraction): Promise<void> {
...
export async function handleStatusUpdate(interaction: ChatInputCommandInteraction): Promise<void> {
  const id = interaction.options.getInteger('id', true);
